...
from Lemmatization.Utils.constants import PAD_TOKEN, EOW_TOKEN, UNK_TOKEN, SOW_TOKEN
from Lemmatization.Utils.dataset import coallate_words, tensors_to_words, LemmaDataSet, VocabCreator
from Lemmatization.evaluation.metrics import Metrics
...
class LemmatizatorEvaluator:
    ...
    def evaluate(self, model: nn.Module, categories: List[str], worst: int = 3) -> None:
        ...
        fig, axs = plt.subplots(nrows=len(self.metrics.metrics) + 1, ncols=len(categories), figsize=(8, 17))
        fig.tight_layout()
